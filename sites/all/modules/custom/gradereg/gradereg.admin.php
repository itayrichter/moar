...
function gradereg_course_export($node){
	...
	//$query->join('attendance', 'attendance', "attendance.course_nid=field_course.field_course_target_id and attendance.student_uid=users.uid");
	...
	$query->addField('grades_data', 'nid', 'grades_data_nid');
	//$query->addField('attendance', 'attendance_status');
	...
	for($i=0; $i<$hm_num; $i++){
		...
		$header[] = t('Attended in class !class', array('!class' => ($i+1)));
	}
	...
	foreach($find as $r){
      ...
      $r->mail,
		...
		for($i=0; $i<$hm_num; $i++){
			...
			$attended = db_query("select attendance_status from {attendance} where student_uid=:uid and course_nid=:nid and lesson_num=:num", array(':uid' => $r->uid, ':nid' => $node->nid, ':num' => ($i+1)))->fetchField();
			$row[] = ($attended == '1' || $attended == '2') ? t('Yes') : t('No');
		}
